...
export const CompendiumScreen: React.FC<CompendiumProps> = ({ onBack, metaState, onBuy }) => {
    ...
    const [selectedTab, setSelectedTab] = useState<'WEAPONS' | 'ARTIFACTS' | 'UPGRADES'>('WEAPONS');
    ...
    const triggerBurst = (e: React.MouseEvent | React.TouchEvent, color: string) => {
        let x = 0, y = 0;
        if ('clientX' in e) {
    ...
    useEffect(() => {
        if (recentlyPurchasedId) {
            const timer = setTimeout(() => setRecentlyPurchasedId(null), 2000); // 2s highlight
            return () => clearTimeout(timer);
        }
    }, [recentlyPurchasedId]);

    const prevUnlockedItems = useRef<string[]>(metaState.unlockedItems);
    const prevPermUpgrades = useRef<Record<string, number>>({ ...metaState.permanentUpgrades });

    // Effect to detect new item purchases and trigger animation
    useEffect(() => {
        const newUnlocks = metaState.unlockedItems.filter(id => !prevUnlockedItems.current.includes(id));
        if (newUnlocks.length > 0) {
            const lastUnlocked = newUnlocks[newUnlocks.length - 1];
            setRecentlyPurchasedId(lastUnlocked);

            // Play success sound
            import('../services/audioEngine').then(({ audioEngine }) => {
                audioEngine.playPurchaseSuccess();
            });
        ...
        prevUnlockedItems.current = metaState.unlockedItems;
    }, [metaState.unlockedItems]);

    // Effect to detect permanent upgrade purchases and trigger animation
    useEffect(() => {
        let purchasedId: string | null = null;
        for (const [id, level] of Object.entries(metaState.permanentUpgrades || {})) {
            const prevLevel = prevPermUpgrades.current[id] || 0;
            if (level > prevLevel) {
                purchasedId = id;
                break;
            }
        ...
        if (purchasedId) {
            setRecentlyPurchasedId(purchasedId);
            ...
            prevPermUpgrades.current = { ...metaState.permanentUpgrades };
            return () => clearTimeout(timer);
        }
        prevPermUpgrades.current = { ...metaState.permanentUpgrades };
    }, [metaState.permanentUpgrades]);
    ...
    const renderItem = (item: any) => {
        ...
        // Animation for recently purchased
        if (recentlyPurchasedId === item.id) {
            containerClass += " animate-purchase-pulse z-20 ring-4 ring-green-400/50";
        }
        ...
        return (
            ...
            >
                ...
                <div className="flex items-start gap-4">
                    <div className={isVisible ? (isUnlocked ? '' : 'grayscale') : 'grayscale'}>
                        {isVisible ? (
                    ...
                    <div className="flex-grow pr-16">
                        ...
                        {isVisible && <div className="text-[10px] font-mono text-gray-500 mb-2 uppercase">{item.type.replace('_', ' ')}</div>}
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-800 flex justify-between items-center">
                        ...
                        <NeonButton
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                if (canUnlock) {
                                    triggerBurst(e, "#4ade80"); // Green burst
                                    onBuy(item.id, item.unlockCost || 0);
                                }
                            }}
                            disabled={!canUnlock}
