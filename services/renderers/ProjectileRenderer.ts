...
export const drawProjectiles = (
    ...
    projectiles.forEach(p => { 
        ...
        if (p.sourceWeaponId === 'drum_echo') {
            ctx.save(); ctx.translate(p.pos.x, p.pos.y); ctx.rotate(frame * 0.05); ctx.strokeStyle = p.color; ctx.lineWidth = 1.5;
            ctx.beginPath(); ctx.ellipse(0, 0, p.radius, p.radius * 0.4, 0, 0, Math.PI * 2); ctx.stroke();
            ctx.beginPath(); ctx.ellipse(0, 0, p.radius, p.radius * 0.4, Math.PI / 3, 0, Math.PI * 2); ctx.stroke();
            ctx.beginPath(); ctx.ellipse(0, 0, p.radius, p.radius * 0.4, (Math.PI / 3) * 2, 0, Math.PI * 2); ctx.stroke();
        ...
        } else if (p.sourceWeaponId === 'spirit_lance') {
            ...
            ctx.restore();
             ...
             const isSonicWall = p.customData?.augment === 'SONIC_WALL';
             
             if (isSonicWall) {
                 ctx.save(); ctx.translate(p.pos.x, p.pos.y); const angle = Math.atan2(p.velocity.y, p.velocity.x); ctx.rotate(angle);
                 const size = p.radius * 4.0; const vibration = Math.sin(frame * 0.75) * 3;
                 const waveCount = 3; const maxDist = 25; const driftSpeed = 3.0;
                 for(let i=0; i<waveCount; i++) {
                     const t = (frame * driftSpeed + i * (maxDist / waveCount)); const xOffset = -(t % maxDist);
                     const progress = Math.abs(xOffset) / maxDist; const alpha = 1 - progress; const height = size * (1 - progress * 0.2);
                     ctx.beginPath(); ctx.strokeStyle = `rgba(0, 255, 255, ${alpha})`; ctx.lineWidth = 2 + (1-alpha)*2; ctx.lineCap = 'round'; ctx.shadowBlur = 10 * alpha; ctx.shadowColor = '#00FFFF';
                     ctx.moveTo(-5 + xOffset, -height); ctx.quadraticCurveTo(5 + xOffset + vibration, 0, -5 + xOffset, height); ctx.stroke();
                 }
                 ctx.fillStyle = '#FFFFFF'; ctx.shadowColor = '#00FFFF'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.ellipse(0, 0, 4, size * 0.8, 0, 0, Math.PI*2); ctx.fill(); ctx.shadowBlur = 0; ctx.restore();
             } else {
                 ctx.save(); ctx.strokeStyle = p.color; ctx.lineWidth = 2; 
                 ctx.beginPath(); const jaggedness = 5; const startX = p.pos.x - (p.velocity.x * 2); const startY = p.pos.y - (p.velocity.y * 2);
                 ctx.moveTo(startX, startY); const midX = (startX + p.pos.x) / 2 + (Math.random() - 0.5) * jaggedness; const midY = (startY + p.pos.y) / 2 + (Math.random() - 0.5) * jaggedness;
                 ctx.lineTo(midX, midY); ctx.lineTo(p.pos.x, p.pos.y); ctx.stroke();
                 ctx.fillStyle = p.color === '#FF4500' ? '#FFD700' : '#E0FFFF'; 
                 ctx.beginPath(); ctx.arc(p.pos.x, p.pos.y, 3, 0, Math.PI*2); ctx.fill(); ctx.restore();
             }
