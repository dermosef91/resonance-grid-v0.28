...
export const drawPickup = (ctx: CanvasRenderingContext2D, p: Pickup, frame: number, viewBounds: { left: number, right: number, top: number, bottom: number }) => {
    ...
    if (p.kind === 'KALEIDOSCOPE') {
        ...
        if (p.kind === 'HEALTH') {
            ...
            const alpha = 0.6 + Math.sin(frame * 0.15) * 0.2;
            const r = p.radius;

            ctx.save();

            // Cheap radial gradient glow (replaces expensive shadowBlur)
            const glowR = (r * 2) + pulse * (r * 0.8);
            const glow = ctx.createRadialGradient(p.pos.x, p.pos.y, 0, p.pos.x, p.pos.y, glowR);
            glow.addColorStop(0, p.color);
            glow.addColorStop(0.4, p.color.slice(0, 7) + '66');
            glow.addColorStop(1, p.color.slice(0, 7) + '00');
            ctx.fillStyle = glow;
            ctx.globalAlpha = 0.5;
            ctx.beginPath();
