...
export const useGameEngine = (
  ...
  const startGame = useCallback(() => {
      ...
      setStatus('PLAYING');
      audioEngine.start(); 
      trackEvent(runIdRef.current, 'RUN_START', playerRef.current, metaState, 1, 0);
  }, [resetGame, metaState]);
  ...
  const gameOver = useCallback(() => {
      ...
      trackEvent(runIdRef.current, 'DEATH', playerRef.current, metaState, waveIndexRef.current + 1, sessionCurrencyRef.current, { causeOfDeath: lastDamageSourceRef.current });
  ...
  const update = useCallback(() => {
    ...
    } else if (tutorialStep === 'COMBAT') { if (killsRef.current > 0) setTutorialStep('COLLECT');
    ...
    if (missionResult.isComplete && waveIndexRef.current < runWaves.length - 1) {
        ...
        if (missionCompleteTimerRef.current === 0) {
            trackEvent(runIdRef.current, 'WAVE_COMPLETE', player, metaState, waveIndexRef.current + 1, sessionCurrencyRef.current);
    ...
    pickupResult.collected.forEach(p => {
        if (p.kind === 'KALEIDOSCOPE') {
            player.kaleidoscopeTimer = 600; particlesRef.current.push(createTextParticle(player.pos, "PRISM ACTIVE (10s)", '#FF00FF', 90)); screenShakeRef.current += 10;
             ...
             trackEvent(runIdRef.current, 'LOOT_PICKUP', player, metaState, waveIndexRef.current + 1, sessionCurrencyRef.current, { lootType: p.supplyContent || 'LEVEL_UP' });
             const content = p.supplyContent || 'LEVEL_UP';
             if (content === 'CURRENCY_50') { sessionCurrencyRef.current += 50; particlesRef.current.push(createTextParticle(player.pos, `+50 CHIPS`, '#FFD700', 60)); }
