...
import { 
    ...
    Weapon, UpgradeOption, EntityType, MetaState, MissionState, MissionType, MissionEntity, WaveConfig, ColorPalette, TutorialStep, Shockwave, Replica, Obstacle
} from '../types';
...
export const useGameState = (metaState: MetaState) => {
  ...
  const [gameOverInfo, setGameOverInfo] = useState<{ score: number, level: number, wave: number, chipsEarned: number, newUnlocks: string[], newAvailable: string[] } | null>(null);
